...
// Be more flexible with blacklist syntax

// Any special regexp char will be escaped
var blacklistDirectiveEscape = /[-\/\\^$+?.()|[\]{}]/g;

// All `*` will be expanded into `.*`
var blacklistDirectiveEscapeAsterisk = /\*/g;

// Probably manually entered blacklist directive
var isHandcraftedBlacklistDirective = function(directive) {
    return directive.indexOf('/') !== -1 &&
           directive.indexOf('*') !== -1;
};

var matchBlacklistDirective = function(url, hostname, directive) {
    ...
    var reStr = directive.replace(blacklistDirectiveEscape, '\\$&')
                         .replace(blacklistDirectiveEscapeAsterisk, '.*');
...
µBlock.getNetFilteringSwitch = function(url) {
    ...
    for (;;) {
        if ( netBlacklist.hasOwnProperty(key) ) {
            ...
            i = buckets.length;
            while ( i-- ) {
                if ( matchBlacklistDirective(url, targetHostname, buckets[i]) ) {
...
µBlock.toggleNetFilteringSwitch = function(url, scope, newState) {
    ...
    if ( newState === true ) {
        if ( netBlacklist.hasOwnProperty(key) === false ) {
            netBlacklist[key] = [];
        }
        netBlacklist[key].push(directive);
        this.saveBlacklist();
        return true;
    }

    // Remove from directive list whatever causes current URL to be blacklisted
    var buckets, i;
    for (;;) {
        if ( netBlacklist.hasOwnProperty(key) ) {
            buckets = netBlacklist[key];
            ...
            while ( i-- ) {
                directive = buckets[i];
                if ( !matchBlacklistDirective(targetURL, targetHostname, directive) ) {
                    continue;
                }
                buckets.splice(i, 1);
                ...
                // directive
                if ( isHandcraftedBlacklistDirective(directive) ) {
                    netBlacklist['#'].push('# ' + directive);
                }
            }
            if ( buckets.length === 0 ) {
                delete netBlacklist[key];
            }
        }
        pos = key.indexOf('.');
        if ( pos === -1 ) {
...
µBlock.stringFromBlacklist = function(blacklist) {
    var r = {};
    var i, bucket;
    for ( var key in blacklist ) {
        if ( blacklist.hasOwnProperty(key) === false ) {
            continue;
        }
        bucket = blacklist[key];
        i = bucket.length;
        while ( i-- ) {
            r[bucket[i]] = true;
        }
...
µBlock.blacklistFromString = function(s) {
    var blacklist = {
        '#': []
    };
