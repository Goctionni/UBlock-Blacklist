...
var onVersionReady = function(lastVersion) {
    // Blacklist some key scopes by default
    if ( lastVersion.localeCompare('0.8.6.0') < 0 ) {
        µb.netBlacklist = µb.blacklistFromString(
            µb.stringFromBlacklist(µb.netBlacklist) +
            '\n' +
            µb.netBlacklistDefault
        );
        µb.saveBlacklist();
    }
...
// Blacklist in memory.
// Blacklist parser needs PSL to be ready.
// gorhill 2014-12-15: not anymore

var onNetBlacklistReady = function(netBlacklistRaw) {
    µb.netBlacklist = µb.blacklistFromString(netBlacklistRaw);
    µb.netBlacklistModifyTime = Date.now();
};
...
var onFirstFetchReady = function(fetched) {
    ...
    onNetBlacklistReady(fetched.netBlacklist);
