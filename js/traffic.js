...
var onBeforeRootFrameRequest = function(details) {
    ...
    // If the site is blacklisted, disregard strict blocking
    if ( µb.getNetFilteringSwitch(requestURL) === false ) {
        result = 'ua:blacklisted';
    }
    ...
    if ( result === '' ) {
        result = isTemporarilyBlacklisted(result, requestHostname);
...
var isTemporarilyBlacklisted = function(result, hostname) {
    var obsolete, pos;

    for (;;) {
        obsolete = documentBlacklists[hostname];
        if ( obsolete !== undefined ) {
            if ( obsolete > Date.now() ) {
                ...
                delete documentBlacklists[hostname];
            }
        }
...
var documentBlacklists = Object.create(null);

/******************************************************************************/

exports.temporarilyBlacklistDocument = function(hostname) {
    if ( typeof hostname !== 'string' || hostname === '' ) {
        return;
    }

    documentBlacklists[hostname] = Date.now() + 60 * 1000;
};
